% This version 3 is same as ver 02 but only difference is in taking care of
% border effects while convolution operation.
function [ResultForDisplay,Result,ScalingScalingResultColsubsampled] = WaveletDecomposition_Fun_03(OriginalImage,ScalingCoefficents)
[rows,cols,dims] = size(OriginalImage);

h_s = ScalingCoefficents;
h_w = ComputeWaveletCoefficents(ScalingCoefficents);
FilterLength = length(h_s);
...
temp2 = [];
for i = 1:rows
    temp1 = padarray(OriginalImage(i,:),[0 FilterLength-1],'symmetric','both');
    temp2 = conv(temp1,h_s(end:-1:1));
    ScalingResultRow(i,:) = temp2(FilterLength:end-FilterLength+1);
    temp2 = conv(temp1,h_w(end:-1:1));
    WaveletResultRow(i,:) = temp2(FilterLength:end-FilterLength+1);
end
...
for i = 1:size(ScalingResultRowsubsampled,2)
    temp1 = padarray(ScalingResultRowsubsampled(:,i),[FilterLength-1 0],'symmetric','both');
    temp2 = conv(temp1,h_s(end:-1:1));
    ScalingScalingResultCol(:,i) = temp2(FilterLength:end-FilterLength+1);
    temp2 = conv(temp1,h_w(end:-1:1));
    ScalingWaveletResultCol(:,i) = temp2(FilterLength:end-FilterLength+1);
    temp1 = padarray(WaveletResultRowsubsampled(:,i),[FilterLength-1 0],'symmetric','both');
    temp2 = conv(temp1,h_s(end:-1:1));
    WaveletScalingResultCol(:,i) = temp2(FilterLength:end-FilterLength+1);
    temp2 = conv(temp1,h_w(end:-1:1));
    WaveletWaveletResultCol(:,i) = temp2(FilterLength:end-FilterLength+1);
end
